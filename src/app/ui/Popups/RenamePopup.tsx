...
import { useDirectoryChildrenStore, useFileChildrenStore, useSelectedINodesStore, useRenamePopupStore } from "@/app/global_state/global_state"
import { useEffect } from "react"

const renameINode = async (rename_directory_child: (inode_id: string, new_inode_name: string) => void, rename_file_child: (inode_id: string, new_inode_name: string) => void) => {
...
const RenamePopup = () => {
  const rename_directory_child = useDirectoryChildrenStore((state) => state.renameChild)
  ...
  useEffect(() => {
    ...
    document.getElementById("rename_form")?.addEventListener("submit", renameINodeInvoker)

    return () => {
      window.removeEventListener("mousedown", handleRenameOutsideClick)
      document.getElementById("rename_form")?.removeEventListener("submit", renameINodeInvoker)
    }

  }, [])

  return (
    <div tabIndex={5} id="rename-popup" className="hidden border-8 border-white h-48 w-48 bg-blue-500 fixed top-1/2 left-1/2 w-1/2 transform -translate-x-1/2 -translate-y-1/2">
      <form id="rename_form" className="text-black font-sans">
        Rename
        <input id="rename-input" type="text" className="text-black font-sans font-normal" />
