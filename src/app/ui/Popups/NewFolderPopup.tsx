...
const createNewFolder = async (
  add_directory_child: (inode: INodeType) => void
) => {
...
const NewFolderPopup = () => {
  ...
  const add_directory_child = useDirectoryChildrenStore(
  ...
  useEffect(() => {
    ...
    document
      ...
      ?.addEventListener("submit", createNewFolderInvoker);

    return () => {
      window.removeEventListener("mousedown", handleOutsideClick);
      document
        .getElementById("new-folder-form")
        ?.removeEventListener("submit", createNewFolderInvoker);
    };
  }, []);
