...
const ChildInodeList = ({
  ...
  set_opened_file_id,
  set_opened_file_name,
}: {
  children: Array<INodeType>;
  set_is_file_modal_open: (isOpen: boolean) => void;
  set_opened_file_id: (fileName: string) => void;
  set_opened_file_name: (fileName: string) => void;
}) => {
  const router = useRouter();
  const set_selected_inodes = useSelectedINodesStore(
    (state) => state.setINodes
  );
  ...
  return children.map((child, i) => {
    ...
    const style = clsx("flex flex-row justify-between p-3 hover:bg-gray-500", {
      "border-t": i !== 0,
      "bg-blue-500":
        selected_inodes.find((inode) => inode.id == child.id) != null,
      "hover:!bg-blue-500":
        selected_inodes.find((inode) => inode.id == child.id) != null,
    });
    return (
      <div
        ...
        onDoubleClick={() => {
          if (child.file_type == "directory") {
            ...
            set_opened_file_id(child.id);
            set_opened_file_name(child.name);
          }
        }}
      >
...
export default function INodeList({
  curr_id,
  directory_children,
  file_children,
}: {
  ...
  const set_directory_children = useDirectoryChildrenStore(
  ...
  const [opened_file_id, set_opened_file_id] = useState("");
  const [opened_file_name, set_opened_file_name] = useState("");
  ...
  const state_directory_children = useDirectoryChildrenStore(
    (state) => state.directoryChildren
  );
  const state_file_children = useFileChildrenStore(
    (state) => state.fileChildren
  );
  ...
  return (
    <div id="inode-list">
      <ChildInodeList
        ...
        set_opened_file_id={set_opened_file_id}
        set_opened_file_name={set_opened_file_name}
      />
      <Popups />
      <FileModal
        is_open={is_file_modal_open}
        inode_id={opened_file_id}
        file_name={opened_file_name}
        set_is_open={set_is_file_modal_open}
      />
    </div>
  );
}
